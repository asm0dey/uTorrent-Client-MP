 ...
 * 
 * <p>
 * To execute a request, use {@linkplain #get(String) Http.get("http://w...")} or {@linkplain #post(String) Http.post("http://w...")}, specify at
 * least the {@linkplain HttpClient client} to use for this request and execute it with one of the execution methods.
 * 
 ...
 * 
 * <pre>
 * final String ok = Http.get( &quot;http://somesite.com&quot; ).use( client ).header( &quot;User-Agent&quot;, &quot;HttpClient Wrapper&quot; ).charset( &quot;UTF-8&quot; )
 * 		.followRedirects( true ).customize( new RequestCustomizer() {
 * 			&#064;Override
...
public final class Http {
	...
	/**
	 * Creates a builder object for a POST-request. Supports data and entity modifications.
	 * 
	 * @param url
	...
	/**
	 * Converts a {@linkplain HttpResponse} to a String by calling {@link EntityUtils#toString(HttpEntity)} on its {@linkplain HttpEntity entity}.
	 * 
	 * @param response
	 ...
	 * @return the response body as a String or {@code null}, if no response body exists or an error occurred while converting.
	 */
	...
	public abstract static class HttpRequestBuilder {
		...
		/**
		 * Appends the String representation of each key-value-pair of the given map to this request.
		 * 
		 * @param data
		...
		/**
		 * Adds the given {@linkplain RequestCustomizer request customizer} to this request. All customizers are being applied sequentially just
		...
		/**
		 * Executes this request and returns the content body of the result as a String. If no response body exists, this returns {@code null}.
		 * 
		...
		/**
		 * Executes this request and saves the response stream to a file. The stream is going to be copied if and only if the response was successful
		 * ({@code 2xx}) and a response body exists. If the response code was {@code >= 300}, a {@link FileNotFoundException} is thrown. If no body
		 * exists, this returns {@code false} and no exception is thrown.
		 * 
		 * @param target
		...
		/**
		 * Executes this request and aborts immediately after execution using the {@linkplain HttpUriRequest#abort() abort} method of this request.
		 * 
		 * @throws IOException
		 *             if an error occurs while execution
		 */
